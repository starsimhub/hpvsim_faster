...
class TxSegmented(hpv.tx):
    ...
    def administer(self, sim, inds, return_format='dict'):
        ...
        if self.results_df is None:
            ...
            self.results_df['year'] = sim.res_yearvec
        ...
        for intv_age in self.intv_ages:
            ...
            self.results_df.loc[self.results_df['year'] == sim.yearvec[sim.t], f'treatments_{intv_age[0]}-{intv_age[1]}'] = people.scale[inds_in_age].sum()  # Count total treatments administered
        ...
        if len(averted_cancer_inds) > 0:
            for intv_age in self.intv_ages:
                ...
                self.results_df.loc[self.results_df['year'] == sim.yearvec[sim.t], f'cancers_averted_{intv_age[0]}-{intv_age[1]}'] = people.scale[averted_cancer_inds_in_age].sum()  # Count total treatments administered
